import { getInstance } from '../integrations/auth/index.ts';
import { AuthProvider } from '../integrations/auth/types.ts';
...
export async function googleLogin(c: Context) {
    const authProvider = getInstance({ authProvider: AuthProvider.Google });
...
export async function googleCallback(c: Context) {
    try {
        ...
        const authProvider = getInstance({ authProvider: AuthProvider.Google });
        const userProfile = await authProvider.handleOAuthCallback({ code, state });
...
export async function register(c: Context) {
    try {
        ...
        // Use integration layer for validation
        const authProvider = getInstance({ authProvider: AuthProvider.EmailPassword });
...
export async function login(c: Context) {
    try {
        ...
        // Use integration layer for validation
        const authProvider = getInstance({ authProvider: AuthProvider.EmailPassword });
        await authProvider.authenticateWithEmailPassword({ email, password });
