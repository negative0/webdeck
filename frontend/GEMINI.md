...
**Backend:** Payment integration pre-configured with Stripe at `backend/src/integrations/payment/`. Only implement payment UI when explicitly requested.
...
- Define interfaces: `CreateCheckoutSessionPayload` (amount: number, currency?: string, productName: string, customerEmail?: string, metadata?: Record<string, any>), `CheckoutSession` (sessionUrl: string, sessionId: string)
- Export async function `createCheckoutSession(payload)`:
    - Always call backend API: (the specific api endpoint that you have created for creating the checkout session) and return response.data
...
export async function createCheckoutSession(payload) {
    // Always use real Stripe API (test mode) - no mock mode for payments
...
const handleCheckout = async () => {
    const { sessionUrl } = await createCheckoutSession({ amount, productName, currency: 'usd' });
...
- **Amount format**: Pass amounts in dollars (29.99) - backend converts to cents
- **Session ID**: Available as `?session_id=cs_xxx` query parameter in success URL
- **Webhook handling**: All payment status updates happen server-side
